...
{
    [AutoloadBossHead]
    public class Holyrizer : ModNPC
    {
        public static int secondStageHeadSlot = -1;
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Holyrizer");
        ...
        public override void ModifyNPCLoot(NPCLoot npcLoot)
        {
            ...
            //}
            npcLoot.Add(ItemDropRule.Common(Mod.Find<ModItem>("SoulOfWight").Type, 1, 20, 50));
        }
        ...
        public override void AI()
        {
                ...
                if (NPC.ai[1] == 0f)
                {
                    ...
                    else if (NPC.position.Y + NPC.height < Main.player[NPC.target].position.Y && num496 < 400f)
                    {
                        ...
                        if (NPC.ai[3] >= 40f)
                        {
                            ...
                            if (Main.netMode != 1)
                            {
                                var num499 = (NPC.type == ModContent.NPCType<Saturationator>()) ? 49 : 20;
                                ...
                                Projectile.NewProjectile(NPC.GetProjectileSpawnSource(), vector46.X, vector46.Y, num494, num495, ModContent.ProjectileType<Projectiles.SeraphFireball>(), num499, 0, Main.myPlayer, 0, 0f);
                            }
                        }
                    }
                }
            ...
            else
            {
                ...
                if (NPC.ai[1] == 0f)
                {
                    ...
                    if (NPC.localAI[1] > 100f && Collision.CanHit(NPC.position, NPC.width, NPC.height, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height))
                    {
                        //SecondStage = true;
                        NPC.localAI[1] = 0f;
                        var num513 = (NPC.type == ModContent.NPCType<Saturationator>()) ? 50 : 25;
                        ...
                        Projectile.NewProjectile(NPC.GetProjectileSpawnSource(), vector48.X, vector48.Y, num509, num510, ModContent.ProjectileType<Projectiles.SeraphFireball>(), num513, 0f, Main.myPlayer, 0f, 0f);
                        return;
                    }
                    return;
                }
                else
                {
                    ...
                    if (Main.netMode != 1)
                    {
                        ...
                        if (NPC.localAI[1] > 60f && Collision.CanHit(NPC.position, NPC.width, NPC.height, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height))
                        {
                            ...
                            var num521 = 9f;
                            var num522 = 18;
                            var num523 = ModContent.ProjectileType<Projectiles.SeraphFireball>();
                            ...
                            Projectile.NewProjectile(NPC.GetProjectileSpawnSource(), vector49.X, vector49.Y, num518, num519, num523, num522, 0f, Main.myPlayer, 0f, 0f);
                        }
                    }
                    NPC.ai[2] += 1f;
                    if (NPC.ai[2] >= 180f)
        ...
        {
            NPC.SetEventFlagCleared(ref DownedBossSystem.downedSaturationator, -1);
