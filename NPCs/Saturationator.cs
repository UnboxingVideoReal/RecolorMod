...
{
    [AutoloadBossHead]
    public class Saturationator : ModNPC
    {
        public static int secondStageHeadSlot = -1;
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Saturationator");
                    ...
                    else if (NPC.position.Y + NPC.height < Main.player[NPC.target].position.Y && num496 < 400f)
                    {
                        ...
                        if (NPC.ai[3] >= 40f)
                        {
                            ...
                            if (Main.netMode != 1)
                            {
                                ...
                                Projectile.NewProjectile(NPC.GetProjectileSpawnSource(), vector46.X, vector46.Y, num494, num495, ModContent.ProjectileType<Projectiles.SaturationLaser2>(), num499, 0, Main.myPlayer, 0, 0f);
                            }
                        }
                    }
                }
            ...
            else
            {
                ...
                if (NPC.ai[1] == 0f)
                {
                    ...
                    if (NPC.localAI[1] > 100f && Collision.CanHit(NPC.position, NPC.width, NPC.height, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height))
                    {
                        ...
                        Projectile.NewProjectile(NPC.GetProjectileSpawnSource(), vector48.X, vector48.Y, num509, num510, ModContent.ProjectileType<Projectiles.SaturationLaser2>(), num513, 0f, Main.myPlayer, 0f, 0f);
                        return;
                    }
                    return;
                }
                else
                {
                    ...
                    if (Main.netMode != 1)
                    {
                        ...
                        if (NPC.localAI[1] > 60f && Collision.CanHit(NPC.position, NPC.width, NPC.height, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height))
                        {
                            ...
                            var num521 = 9f;
                            var num522 = 18;
                            var num523 = ModContent.ProjectileType<Projectiles.SaturationLaser2>();
