...
{
	public class BambiSltuoj : ModItem
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Bambi Solution");
		...
		{
			Item.shoot = ModContent.ProjectileType<BambiSltuojProj>() - ProjectileID.PureSpray;
			Item.ammo = AmmoID.Solution;
			Item.width = 12;
			Item.height = 14;
			Item.value = Item.buyPrice(0, 0, 0, 0);
			Item.rare = ModContent.RarityType<Bambi>();
			Item.maxStack = 999;
			Item.consumable = true;
		}

		public class BambiSltuojProj : ModProjectile
		{
			...
			public override void SetStaticDefaults()
			{
				DisplayName.SetDefault("Bambi Spray");
			}
				...
				for (int k = i - size; k <= i + size; k++)
				{
					for (int l = j - size; l <= j + size; l++)
					{
						if (WorldGen.InWorld(k, l, 1) && Math.Abs(k - i) + Math.Abs(l - j) < Math.Sqrt((size * size) + (size * size)))
						{
							...
							{
								Main.tile[k, l].TileType = (ushort)ModContent.TileType<BambiGrass>();
								WorldGen.SquareTileFrame(k, l);
								NetMessage.SendTileSquare(-1, k, l, 1);
							}
							if (TileID.Sets.Conversion.Stone[type])
							{
								Main.tile[k, l].TileType = (ushort)ModContent.TileType<Bambistone>();
								WorldGen.SquareTileFrame(k, l);
								NetMessage.SendTileSquare(-1, k, l, 1);
							}
							//if (TileID.Sets.Conversion.Stone[type])
